...
def adjust_bright(img: Image) -> Image:
    """RGB 각 채널의 평균을 128로 만듭니다."""
    return PIL.Image.fromarray(np.clip(np.array(img, dtype=np.uint16) + 128 - np.mean(img), 0, 255).astype(np.uint8))
...
class SquareCrop:
    def __call__(self, image):
        ...
        return F.center_crop(image, (img_size, img_size))


class SquareReflectPad_Tensor:
    """need to transform to tensor
    ...
    transforms.ToTensor(),
    SquareReflectPad_Tensor(),
    ...
    def __call__(self, image):
        ...
        if big < small:
                big, small = small, big
        diff = big - small
        padded_img = image.unsqueeze(0)
        for i in range(10):
            if diff >= small:
                p_size = small - 1
            else:
                p_size = diff
            paddings = (0, 0, 0, p_size) if wide else (0, p_size, 0, 0)
            padded_img = torch.nn.ReflectionPad2d(paddings)(padded_img)
            if diff > small:
                diff -= p_size
            else:
                break
...
class SharpnessRandom:
    def __init__(self, mag=20, prob=0.8):
        self.mag = mag
        self.prob = prob
    ...
    def __call__(self, image):
        if (self.prob < 1.0) and (random.random() > self.prob):
            return image
        return PIL.ImageEnhance.Sharpness(image).enhance(
            1 + self.mag
        )
