...
class MySpecialDataset(ImageFolder):  
    ...
    def __getitem__(self, index):
        ...
        im = torch.as_tensor(t2im, dtype=torch.uint8, device='cpu')

        im = im.permute(1, 2, 0)
        im = im.numpy()
#         filling = max(im.shape[0],im.shape[1])-min(im.shape[0],im.shape[1])
#         add1 = filling//2
#         add2 = filling - add1
#         fill = int(max(im.shape[0],im.shape[1])*0.1)
#         C = [0, 0, 0]
#         if im.shape[0] > im.shape[1]:
#             im = cv2.copyMakeBorder(im,fill,fill,add1+fill,add2+fill,cv2.BORDER_WRAP, value=C)
#         else:
#             im = cv2.copyMakeBorder(im,add1+fill,add2+fill,fill,fill,cv2.BORDER_WRAP, value=C)
...
def get_dataset(
    ...
    if dataset_name == "TACO":
        ...
        train_dataset_4 = ImageFolder(root=train_path, transform=transform_train)
        train_dataset_2 = MySpecialDataset(train_path, transform = transform_train_albu,t2=transform_albu)
        train_dataset_3 = MySpecialDataset(train_path, transform = transform_train_albu,t2=transform_albu)
        alist = []
        print("Start make dataset")
        print(len(train_dataset_4))
        for i, (data, label) in enumerate(train_dataset_4):
            if(i%500 == 0): print(i)
            if label in [1,6]: alist.append(i)
        train_dataset_4 = Subset(train_dataset_4, alist)
        print(len(train_dataset_1))
        print(len(train_dataset_4))
        train_dataset = ConcatDataset([train_dataset_2, train_dataset_3])
